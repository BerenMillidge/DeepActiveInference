...
function replay(opt_v::ADAM, opt_t::ADAM, memory, valuenet, deep_value_net, Tmodel)
  ...
  for (iter, item) in enumerate(minibatch)
    ...
    ep_bonus::Float32 = sum((item.next_state .- Tmodel(vcat(item.state,action)).data)::Array{Float32,1} .^ 2)
    #println(ep_bonus)
    target += ep_bonus
    if !item.done
...
function save_results()
    ...
    for i in 1:20
      ...
      push!(vls, vlosses)
      push!(tls, tlosses)
      BSON.bson("results/lunar_lander_active_inference_Tmodel.bson", a=[rs,pls,vls,tls])
